...
//    Port_Init(); 
    ...
    printf("**************************************************\n");
    printf("*                                                *\n");
    printf("* PMC, LLWU and MC Tower Demo                    *\n");
    printf("*                                                *\n");
    printf("**************************************************\n");
    printf(HELPMSG);
    printf("\n\n");
...
void enter_vlpr_mode (int argc, char **argv)
{
    ...
    if (argc != 2){
       ...
       if (setclr_lpwui) {
          MC_PMCTRL = MC_PMCTRL_LPWUI_MASK | MC_PMCTRL_RUNM(2) ;      //enter VLPR with write to RUNM = 10
          printf(" LPWUI = 1 return to RUN Mode.\n\r");
       } else {
          MC_PMCTRL = !MC_PMCTRL_LPWUI_MASK | MC_PMCTRL_RUNM(2) ;     //enter VLPR with write to RUNM = 10  LPWUI = 0
          printf(" LPWUI = 0 return to VLPR Mode \n\r");
       }
       printf("MC PMCTRL= %#02X \r\n", (MC_PMCTRL) ) ;
       printf("PMC_REGSC = %#02X \r\n\n", (PMC_REGSC) ) ;

       printf("\n\n\n To communicate in VLPR - Change Baud Rate to 19200 baud NOW !!!! \n\r");
       simclkdiv =  SIM_CLKDIV1;      // save the current setting.

       mcg_pee_2_blpi();  // Must change system clock to < 2MHz to enter VLP modes.
       ...
       } //while VLPRS clear - not in low power run mode yet - wait


       for (i= 0;i<0xffff;i++){
           printf("                   Now in VLPR at 19200 baud  - hit a key to continue.\r");
              if ((UART_S1_REG(TERM_PORT) & UART_S1_RDRF_MASK)) {
                 break;
              }//if char received  */
       }//for
