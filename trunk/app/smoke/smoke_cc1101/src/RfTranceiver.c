 ...
 * ********************************************************************** */
 ...
 #include "../Components/Hal/Rfcc1101.h"
 ...
 //transmit tx power 
 static unsigned char          TxPower = Rf_Transmit_Level5;
 //cc1101 reset call back function  
 static RfResetCallBack        pRfResetCallBack = NULL;
 
 
 //extend interrupt0 insert address
 ...
 * ============================================== */
 ...
 void CC1101_Config_Pins(void)
 {
	...
	//sclk
	SCLK_OUTPUT;         //output
	CC1101_Sclk_High;     //low
	//gdo0
	GDO0_INPUT;          //output
	CC1101_Gdo0_High;    //low
    #ifdef CSMA_CA
	   //gdo2
	   GDO2_INPUT;          //input
	   CC1101_Gdo2_High;    //low
    #endif
 }
 ...
 * ============================================== */
 ...
 {
    //config interupt pin 
    RPINR0bits.INT1R = Rf_Interrupt_Pin;
    //clean interrupt isr flag
    Clean_Interrupt_ISR_Flag;
    //config interupt level
    IPC5bits.INT1IP   = priority;
    //Up level interrupt 
    Enable_DownLevel_Interrupt;
    //enable interrupt 
    Enable_Interrupt;
 }
 ...
 * ============================================== */
 ...
 {
    //cc1101 power down 
    CC1101_Entry_XMode(cc1101_SPWD);
 }
 ...
 * ============================================== */
 void CC1101_WakeUpToIdleMode(void)
 {
    ...
    //rf Select transmit power
    CC1101_Select_Transmit_Power_Level(TxPower);
 }
 ...
 void CC1101_Initialization(void)
 {
    ...
    //setting rf frequency section
    Rf_Setting_FreSection(RF_ISM_868MHz);
    //set rf default carrier sense threshold
    Rf_CarrierSense_Threshold(60);
    //setting rf channel
    Rf_Setting_Channel(0);
    //setting rf power 
    Rf_Select_Transmit_Power_Level(5);

    //注意这里的地址,此机本地地址为0x01 ，那另外一台机地址最好不要设置相同的地址。
    Setting_CC1101_Address(0x01);
