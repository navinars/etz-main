...
#define my_delay_ms(_x) vTaskDelay(_x)
...
UINT16 gRecieveIndex = 0;
UINT8 gRecieveFlag = 0;
...
{     
   ...
   /* Construct the AT command */
   sprintf(at_cmd_buf,"AT+DHCPSRVR=%d\r\n",mode);
...
*/
void AtLib_SendTcpData( UINT8 cid,UINT8 *txBuf, UINT32 dataLen)
...
*/
HOST_APP_MSG_ID_E AtLib_checkEOFMessage(UINT8 * pBuffer)
{
	...
	/* 判断TCP/UDP连接成功，获取UID。
	   TCP客户端连接显示CONNECT UID
	   TCP服务器端显示CONNECT UID UID IP地址，以OK结尾*/
	else  if(((UINT8 *)strstr((const char *)pBuffer,"CONNECT")) != NULL)
	{
#if AP
		AtLib_ParseUdpServerStartResponse(&wifiInfo.tcp_uid);
		return HOST_APP_MSG_ID_NONE;
#else
		AtLib_ParseTcpClientCid();
		return HOST_APP_MSG_ID_NONE;
#endif
	}
...
{/* Note: This function can be called from ISR, if host wake-up GPIO interrupt is supported.
   ...
   */
	...
	while(BSP_Uart0_Read(&rxData,1) == UART_YES_DATA)  /* Read one byte at a time - Use  non-blocking call  */
	{
		/* Process the recieved data */
		AtLib_RecieveDataProcess(rxData);
	}
	...
	switch(recieve_state)
	{
		...
		case HOST_APP_RX_STATE_DATA_HANDLE:
		{
			...
			if(rxData == HOST_APP_DATA_MODE_NORMAL_END_CHAR_E)
			{
				...
				{
					gRecieveFlag = 1;
					memcpy(gRecieveBuffer, MRBuffer, (MRBufferIndex-2));
					MRBufferIndex = 0;
					recieve_state = HOST_APP_RX_STATE_START;
				}
				else
...
HOST_APP_MSG_ID_E AtLib_ProcessRxChunk(UINT8 *rxBuf, UINT32 bufLen)
{
	HOST_APP_MSG_ID_E rxMsgId;
...
void AtLib_ConvertNumberTo4DigitASCII(UINT32 myNum, INT8 *pStr)
...
void AtLib_ProcessIncomingData(UINT8 cid, UINT8 rxData)
{
	if(gRecieveBuffer[0] != '\0')
	{
		gRecieveBuffer[gRecieveIndex] = rxData;
		gRecieveIndex ++;
	}
	else
	{
		gRecieveBuffer[0] = cid;
	}
}
